...
export class ToastService {
  constructor(
    ...
    // load sound
    this.toastAudio.src = '/assets/audio/toast.mp3';
  ...
  playAudio(type: 'toast' | 'errorToast') {
    if (type === 'toast') {
      ...
      this.toastErrorAudio.currentTime = 0;
    }
  }

  async presentToast(message: string, duration: number = 2000) {
    let toastElement = await this.toastController.create({
      message,
      duration: duration,
      cssClass: 'agent-toast',
      positionAnchor: 'agent-app-tabBar',
      buttons: [
        {
          text: 'OK',
          role: 'cancel',
  ...
  async presentLoading(message: string) {
    let toastElement = await this.loadingController.create({
      message,
    });
    await toastElement.present();
    return toastElement;
